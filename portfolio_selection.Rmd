...
df.pareto.subset =  subset(df.pareto,Rebalance.Period == "Yearly")

plot.dd = ggplot() + 
          geom_point(data = df.lazy.subset,aes(x = as.numeric(levels(DD))[DD],y = as.numeric(CAGR),colour = Name),size = 2) +
...
plot.md = ggplot() + 
          geom_point(data = df.lazy.subset,aes(x = as.numeric(levels(MD))[MD],y = as.numeric(CAGR),colour = Name),size = 2) +
         # scale_colour_brewer(palette = "Spectral") +
          theme_dark() +
          scale_y_continuous(labels = percent) + 
          ...
          labs(title = "Annualized Returns vs Maximum Drawdown",
            x = "Maximum Drawdown",
            y = "CAGR") + 
          facet_grid(. ~ Rebalance.Period) +
          theme(legend.title=element_blank(),legend.position = "bottom")


plot.pi = ggplot() + 
          geom_point(data = df.lazy.subset,aes(x = as.numeric(levels(PI))[PI],y = as.numeric(CAGR)),colour = "Gray",size = 2) +
          geom_point(data = df.pareto.subset,aes(x = as.numeric(levels(PI))[PI],y = as.numeric(CAGR),colour = Name),size = 2) +
...
```{r plot,echo = false, fig.width = 11, fig.height = 11}
